...
const BestCars: React.FC<BestCarProps> = ({setBrands}) => {
  ...
  useEffect(() => {
   ...
   setH1(typeof messages[messages?.length-2]?.message==="string" && messages[messages?.length-2]?.message  || 'Best Cars for You')
  }, []); 
        ...
        <Table>
          <TableHead>
            <TableRow sx={{ backgroundColor: '#424242' }}>
              ...
              <TableCell sx={{ color: 'white' }}>Price Range (INR, lakh)</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {
              ...
              <TableRow key={index}>
                <TableCell>
                  <Button
                    size="small"
                    variant="outlined"
                    onClick={() => handleMenuClick({brand:keyData, models:tableData[keyData]?.Models})}
                    sx={{ textTransform: 'none', fontSize: '14px' }}
                  >
                    {keyData}
                  </Button>
                </TableCell>
                <TableCell>{tableData[keyData]?.Models?.join(',')}</TableCell>
                <TableCell>{tableData[keyData]?.['Mileage (km/l)']?.join(',')}</TableCell>
                <TableCell>{tableData[keyData]?.['Price Range (INR, lakh)']?.join([','])}</TableCell>
              </TableRow>
            ))
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
        {bottomMenuItems.map((item, idx) => (
          <Button
            ...
            sx={{ textTransform: 'none', fontSize: '14px', padding: '5px 10px' }}
          >
            {item}
          </Button>
        ))}
